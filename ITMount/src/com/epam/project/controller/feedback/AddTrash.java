...
public class AddTrash implements Action{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		if(list!=null){
			for (String string : list) {
				...
				if( feedback.getActive()){
					feedback.setActive(false);
					FeedbackService.updateFeedback(feedback);
					status = "success";
					message =res.getString("Feedback.message.addTrash");
				}else{
					FeedbackService.delFeedback(id);
					status = "success";
					message =res.getString("Feedback.message.delete");
				}
			}
