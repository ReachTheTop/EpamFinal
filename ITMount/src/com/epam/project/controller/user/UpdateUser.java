...
public class UpdateUser implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		current_user.setEmail(request.getParameter("email"));
			...
			if (image.getSize()>0) {
				String fileName = m.uploadFile(image, request.getServletContext(),"user_id"+current_user.getId());
				if(current_user.getImage()!=null){
					DeleteFile.deleteFile(current_user.getImage(), request.getServletContext());
				}
			...
			if (cv.getSize()>0) {
				...
				String fileName = m.uploadFile(cv, request.getServletContext(),"user_id"+current_user.getId());
				if(current_user.getCurriculum_vitae()!=null){
					DeleteFile.deleteFile(current_user.getCurriculum_vitae(), request.getServletContext());
				}
