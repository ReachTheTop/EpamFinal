...
public class ContactDAO {
	...
	private static final String GET_BY_USER_ID = "SELECT * FROM contact WHERE user_id = ?;";
	...
	public static void updateContact(Contact contact, Connection connection) {
		try {
			...
			PreparedStatement st = connection
					.prepareStatement(SQL_UPDATE_CONTACT);
	...
	public static List<Contact> getAllContact(Connection connection) {
		...
		try {
			...
			PreparedStatement st = connection
					.prepareStatement(SELECT_ALL_CONTACT);
			rs = st.executeQuery();
	...
	public static Contact getByUserId(Connection connection, Integer user_id) {
		PreparedStatement ps = null;
		ResultSet rs = null;
		Contact contact = null;
		try {
			ps = connection.prepareStatement(GET_BY_USER_ID);
			ps.setInt(1, user_id );
			rs = ps.executeQuery();
			contact = ContactTransformer.getContact(rs);
		} catch (SQLException e) {
